...
/**
 * The main class of the game. Contains the main loop and pieces everything together. This file
 * should ideally be as short as possible.
 * 
 * @author Robert
 *
 */
public class Game {
   ...
   */
  private final TextUIElement fpsUI = new TextUIElement(InterfacePosition.TOP_LEFT, Color.YELLOW,
      24, TextStyle.BOLD);
  /**
  ...
  public void run() throws FileNotFoundException {
    ...
    long nextUpdate = updateClock.getElapsedTime().asMilliseconds(); // Calculate next update time
                                                                     // in milliseconds
    while (window.isOpen()) { // Run main loop as long as window is open
      handleInput(); // Process input
      long updateTime = updateClock.getElapsedTime().asMilliseconds(); // Make note of the current
                                                                       // update time
  ...
  public void handleInput() {
          ...
          if (playerInventory.isVisible()) {
            switch (event.asKeyEvent().key) {
              ...
              case D:
                ...
                break;
              case E:
                playerInventory.toggleActionMenu();
                break;
              case R:
                playerInventory.addItem(new Item(Item.ItemType.FISH));
                playerInventory.addItem(new Item(Item.ItemType.HEALTH_POTION));
                break;
              case T:
    ...
    /*
     * Real-time input (no lag) Good for repeated actions, bad for single-press actions
     */
