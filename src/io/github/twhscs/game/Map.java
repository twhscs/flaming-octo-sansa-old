...
/**
 * A map or level for the player to interact in. Can be created manually or
 * procedurally.
 * 
 * @author Robert
 *
...
public class Map implements Drawable {
	...
	/**
	 * The vertex array for the tile map. This is a streamlined way to draw many
	 * sprites at once. By using the vertex array instead of 100s of sprites,
	 ...
	 *            Map width.
	 * @param t
	 *            Default tile to auto-populate map.
	 */
	public Map(int l, int w, Tile t) {
		dimensions = new Vector2i(l, w); // Update map dimensions
		tileArray = new Tile[dimensions.x][dimensions.y]; // Create a new tile
															// array with the
															// new size
	 ...
	 * 
	 * @param initialTile
	 *            The tile to populate the map with.
	 */
	public void initializeMap(Tile initialTile) {
		/*
		 * Arrays.fill does not work with matrices Therefore I have to loop
		 * through the outer level to fill the inner level
		 */
		for (Tile[] row : tileArray) {
			Arrays.fill(row, initialTile); // Set each tile in the row to the
											// specified tile
		}
	}
	...
	public void draw(RenderTarget target, RenderStates states) {
		...
		for (int i = playerPos.x - 30; i < playerPos.x + 30; i++) {
			for (int j = playerPos.y - 30; j < playerPos.y + 30; j++) {
				if ((i >= 0 && j >= 0) && (i<dimensions.x && j<dimensions.y)) {
					if (tileArray[i][j] != null) {
						Tile tileType = tileArray[i][j]; // Grab the current
															// tile in
															// the
															// loop
						...
						vertexArray.add(new Vertex(new Vector2f(i * tileSize, j * tileSize), textureCoords)); // Top-left

						vertexArray.add(new Vertex(new Vector2f(i * tileSize, (j * tileSize) + tileSize), Vector2f.add(
								textureCoords, new Vector2f(0, tileSize)))); // Bottom-left

						vertexArray.add(new Vertex(new Vector2f((i * tileSize) + tileSize, (j * tileSize) + tileSize),
								Vector2f.add(textureCoords, new Vector2f(tileSize, tileSize)))); // Bottom-right

						vertexArray.add(new Vertex(new Vector2f((i * tileSize) + tileSize, j * tileSize), Vector2f.add(
								textureCoords, new Vector2f(tileSize, 0)))); // Top-right
					}
				}
			}
		}
	 ...
	 * 
	 * @param l
	 *            The location of the tile to get.
	 * @return The tile at the specified location.
	 */
	public Tile getTile(Location l) {
		Vector2i position = l.getPosition(); // Get the position from the
												// location
		Tile t = tileArray[position.x][position.y]; // Get the tile at this
													// position
	...
	public boolean isValidLocation(Location l) {
		Vector2i coordinates = l.getPosition(); // Get the position from the
												// location
		// Return true if the location is greater than 0, 0 and less than l, w
		return ((coordinates.x >= 0) && (coordinates.y >= 0) && (coordinates.x < dimensions.x)
				&& (coordinates.y < dimensions.y) && Tile.getCanWalkOn(getTile(l)) && (getEntityatLocation(l) == null));
	}
	...
	public Entity getEntityatLocation(Location l) {
		Iterator<Entity> it = entitiesOnMap.iterator(); // Iterate through
														// entities
		while (it.hasNext()) {
	...
	public void updateAllEntities() {
		Collections.sort(entitiesOnMap); // Sort entities by y value for proper
											// rendering
		for (Entity e : entitiesOnMap) {
	...
	public Location getRandomValidLocation() {
		Location randLoc = getRandomLocation(); // Get a random location on the
												// map
		while (!isValidLocation(randLoc)) {
			randLoc = getRandomLocation(); // Keep generating new locations
											// until valid
		}
		// Store all possible directions in a list
		List<Direction> directions = Arrays.asList(Direction.values());
		Collections.shuffle(directions); // Shuffle the list
		randLoc.setDirection(directions.get(0)); // Get the first (random)
													// element in the list
		return randLoc; // Return the new location with both a random position
						// and direction
	}
